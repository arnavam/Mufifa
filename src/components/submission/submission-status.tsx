...
export function SubmissionStatus({ isLocked, lockedAt, predictionCount = 0 }: SubmissionStatusProps) {
  if (isLocked) {
    return (
        ...
        <div className="flex items-center space-x-3 mb-4 sm:mb-0">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground flex items-center gap-1 mt-1">
              <Clock className="w-3 h-3" /> Submitted on {lockedAt ? formatDate(lockedAt) : 'Unknown'}
            </p>
          </div>
        </div>
  ...
  return (
    <div className="flex items-center space-x-3 p-4 bg-card/60 backdrop-blur border border-border/50 rounded-lg">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">Your team has not locked in any predictions yet.</p>
      </div>
    </div>
  )
}
